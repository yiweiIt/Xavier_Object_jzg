...
Page({
  ...
  getmachinDateil: function() {
    var that = this;
    app.func.req('machine/machineOuts/wxQueryMachineOutById', {
      id: that.data.machineOutId
    }, function(data) {
      if (data) {
        that.setData({
          machineOutInfo: data,
  ...
  payMoney: function() {
    ...
    app.func.req('wxs/getPayPrepay', {
      ...
      payType: 5,
      machineOutId: that.data.machineOutId
    }, function(data) {
      if (data) {
        console.log(data)
        that.doWxPay(data)
  ...
  // 获取完整手机号
  getContactPhone: function(i) {
    var that = this;
    if(i==0){
      var title = '支付成功';
    }else{
      var title = '获取成功，消耗1积分';
    }
    app.isBind().then(value => {
      app.func.req("machine/machineOuts/getFullPhone", {
        machineOutId: that.data.machineOutId
      }, function(data) {
        if (!data.error) {
          wx.showToast({
  ...
  onShareAppMessage: function(ops) {
    ...
    return {
      title: '建筑港机械出租信息',
      path: '/pages/rentdetails/rentdetails?id=' + that.data.machineOutId,
