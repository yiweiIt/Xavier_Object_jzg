...
Page({
  ...
  gopeopleDateil: function() {
    var that = this;
    app.func.req("machineRecruit/wxQueryMachineRecruitById", {
      id: that.data.machineRecruitId
    }, function(data) {
      if (data) {
        that.setData({
          machineRecruitInfo: data,
          contract: '建筑港',
          phone: data.phone,
  ...
  onShareAppMessage: function(ops) {
    ...
    return {
      title: '建筑港机械手信息',
      path: '/pages/jobdetails/jobdetails?id=' + that.data.machineRecruitId,
  ...
  payMoney: function() {
    ...
    app.isBind().then(value => {
        app.func.req('wxs/getPayPrepay', {
          ...
          payType: 6,
          machineRecruitId: that.data.machineRecruitId
        }, function(data) {
          if (data) {
            console.log(data)
            that.doWxPay(data)
  ...
  getContactPhone: function() {
    var that = this;
    app.func.req("machineRecruit/getMachineRecruitFullPhone", {
      machineRecruitId: that.data.machineRecruitId
    }, function(data) {
      if (!data.error) {
        wx.showToast({
    ...
    wx.login({
      success: res => { // 发送res.code 到后台换取 openId, sessionKey, unionId
        app.func.req('hireReports/addReport', {
          code: res.code,
          machinerRecruitId: that.data.machineRecruitId,
